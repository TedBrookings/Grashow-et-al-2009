...
% FI = GetFICurve(FileName, CellName, PlotVar)
%        ---OR---
% FI = GetFICurve(t, v, I, PlotVar)
...
FI_Analyze = [];
for n = 1:NumTraces
  Analyze = AnalyzeWaveform3(t(On:Off), v(On:Off,n), SpikePlotVar, ...
			     NoShape);
  %keyboard
  STimes = Analyze.Spike.Times;
  Num = length(STimes);
  if(Num == 0)
    Freq = 0;
  elseif(Num == 1)
    ...
    Freq = 1000 * (Num - 1) / (STimes(end) - STimes(1));
  end
  FI_F = [FI_F, Freq];
